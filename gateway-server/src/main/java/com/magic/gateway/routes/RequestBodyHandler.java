...
public class RequestBodyHandler implements RewriteFunction<String, String> {
    private final Map<String, String> route;

    public RequestBodyHandler(Map<String, String> route) {
        this.route = route;
    }
    ...
    public Publisher<String> apply(ServerWebExchange serverWebExchange, String body) {
        ...
        if ("true".equals(encrypted)) {
            ...
            String newBody = gatewayHandler.handle(body, false);
            return Mono.just(newBody);
        }
        else {
            ServerHttpRequest request = serverWebExchange.getRequest();
            HttpHeaders headers = request.getHeaders();
            String bodyCryptHeader = headers.getFirst(GatewayConstants.BODY_ENCRYPTED_HEADER);
            if ("true".equals(bodyCryptHeader)) {
